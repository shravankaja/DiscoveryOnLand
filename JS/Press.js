...
<g>
    ...
    const carouselItems = document.querySelectorAll('.imgitem')
    const carousel = document.getElementById('imgcarousel')
    let counter = 0;

    leftbutton.addEventListener('click', function () {
        counter--;
        console.log(carouselItems[counter].id)
        findAndUpdate(counter,"left")
        carousel.style.transform = 'translateX(' + (-size * counter) + 'px)';
    })

    let size = carouselItems[1].clientWidth;
    ...
    let checkEndSlides = function (counter) {

        if (carouselItems[counter].id == "2" || carouselItems[counter].id == "3") {
            return true;
        }
        else {
            return false;
    ...
    const findAndUpdate = function(count,type) {
        ...
        console.log(carouselItems[countTest].id)

        if(carouselItems[countTest].id == "2") {
            updateById("mainheading","James Island Discovery's Newest Property")
            updateById("desc","Discovery Land Company is excited to announce our 24th private luxury residential community. James Island Golf and Ocean Club, a 780-acre private island in British Columbia with only 76 lots, is Discovery Land Company’s most exclusive community to date and Discovery’s first project in Canada")
            updateById("c","2")
        }

        else if(carouselItems[countTest].id == "3") {
            updateById("mainheading","CostaTerra Discovery's 23rd property")
            updateById("desc", "Discovery Land Company, the world’s leading developer of luxury private residential communities, clubs and resorts, has announced its 23rd property and first in Europe")
            updateById("c","3")
        }
        else if(carouselItems[countTest].id == "4") {
            updateById("mainheading", "Troubadour To Open Golf Course Fall 2019")
            updateById("desc", "Troubadour Golf & Field Club, a Discovery Land Company community, is pleased to announce the opening of its 18-hole Tom Fazio-designed golf course this fall")
            updateById("c","4")
        }
        else if(carouselItems[countTest].id == "1") {
            updateById("mainheading", "Discovery’s newest property in Kaua’i North Shore Preserve")
...
///--------------Press Carousel ---------------------------------------------------------------------------------------------
    ...
    const leftb = document.getElementById('left1')
    const rightb = document.getElementById('right1')
    let counter1 = 1;
    ...
    rightb.addEventListener('click', function () {
        ...
        document.getElementById("c1").innerHTML   = String(counter1)
        UpdateAndFind(counter1,"right") 
    })
    ...
    const UpdateAndFind = function (count1,type) {
        var co = 0
        if(type=="right") {
            co = count1
        }
        ...
        if(co === 2) {
            console.log(true)
            updateById("r1c1","A Rockwellian Lifestyle at Silo Ridge (Silo Ridge)")
        }

       else if(co == 3) {
            updateById("r1c1", "Third")
        }

        else if(co ==1) {
            updateById("r1c1", "How Discovery Land Company's Mike Meldman Develops the World's Most Exclusive Private Clubs and Resort Communities")
        }
    }
